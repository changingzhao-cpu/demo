...
func run() -> Array[String]:
	...
	_test_holder_fixture_bucket_metadata_stays_aligned_with_processed_count(failures)
	...
	_test_probe_intents_match_attacker_side_filter(failures)
	...
	_test_holder_fixture_waiting_grid_neighbors_exclude_waiting_unit_after_motion(failures)
	_test_holder_fixture_success_grid_key_remains_cell_consistent_after_motion(failures)
	_test_holder_fixture_waiting_grid_key_remains_unset_after_motion(failures)
	...
	_test_holder_fixture_assignment_anchor_matches_target_store_position_after_grid_update(failures)
	_test_holder_fixture_assignment_anchor_remains_shared_across_attackers_after_grid_update(failures)
	return failures
...
func _test_holder_fixture_success_cell_key_matches_grid_neighbor_presence(failures: Array[String]) -> void:
	...
	var key = grid.get_cell_key(0)
	_assert_true(key != null and grid.query_neighbors(Vector2.ZERO).has(0), "success grid key should match neighbor presence", failures)
...
func _test_holder_fixture_bucket_metadata_stays_aligned_with_processed_count(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	store.intent_state[0] = Types.INTENT_STATE_ATTACK
	var report: Dictionary = simulation.tick_bucket_with_report(store, 0.1, 0, 1)
	_assert_true(int(report.get("processed", -1)) >= int(report.get("bucket_count", -1)), "holder fixture processed count should stay aligned with bucket metadata", failures)
...
func _test_holder_fixture_contention_waiting_count_never_exceeds_intent_count(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	store.intent_state[0] = Types.INTENT_STATE_ATTACK
	var contention: Dictionary = simulation.tick_bucket_with_report(store, 0.1, 0, 1).get("contention", {})
	_assert_true(int(contention.get("waiting_count", -1)) <= int(contention.get("intent_count", -2)), "holder fixture waiting count should never exceed intent count", failures)

func _test_holder_fixture_contention_contested_groups_never_exceed_intent_count(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	store.intent_state[0] = Types.INTENT_STATE_ATTACK
	var contention: Dictionary = simulation.tick_bucket_with_report(store, 0.1, 0, 1).get("contention", {})
	_assert_true(int(contention.get("contested_groups", -1)) <= int(contention.get("intent_count", -2)), "holder fixture contested groups should never exceed intent count", failures)

func _test_probe_report_exposes_assignment_snapshot(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
...
func _test_probe_report_exposes_intent_snapshot(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
...
func _test_probe_intents_match_intent_report(failures: Array[String]) -> void:
	...
	store.intent_state[0] = Types.INTENT_STATE_ATTACK
	var report: Dictionary = simulation.tick_bucket_with_report(store, 0.1, 0, 1)
	_assert_eq(report.get("probe", {}).get("intents", []), report.get("intents", []), "probe intents should match intent report", failures)
...
func _test_probe_intents_keep_attacker_side_filter(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
...
func _test_probe_intents_match_attacker_side_filter(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	for intent_variant in probe_intents:
		var intent: Dictionary = intent_variant
		_assert_true(int(intent.get("entity_id", -1)) != 2, "probe intents should match attacker-side filter", failures)
...
func _test_probe_assignments_keep_attacker_side_filter(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
...
func _test_holder_fixture_waiting_grid_neighbors_exclude_waiting_unit_after_motion(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	simulation.tick_bucket_with_report(store, 0.1, 0, 1)
	var committed_pos := Vector2(store.position_x[0], store.position_y[0])
	_assert_true(not grid.query_neighbors(committed_pos).has(1), "waiting grid neighbors should exclude waiting unit after motion", failures)

func _test_holder_fixture_success_grid_key_remains_cell_consistent_after_motion(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	simulation.tick_bucket_with_report(store, 0.1, 0, 1)
	var committed_pos := Vector2(store.position_x[0], store.position_y[0])
	_assert_eq(grid.get_cell_key(0), Vector2i(int(floor(committed_pos.x / 10.0)), int(floor(committed_pos.y / 10.0))), "success grid key should remain cell-consistent after motion", failures)

func _test_holder_fixture_waiting_grid_key_remains_unset_after_motion(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
...
func _test_holder_fixture_success_grid_neighbors_track_committed_position_after_motion(failures: Array[String]) -> void:
	...
	simulation.tick_bucket_with_report(store, 0.1, 0, 1)
	var committed_pos := Vector2(store.position_x[0], store.position_y[0])
	_assert_true(grid.query_neighbors(committed_pos).has(0), "success grid neighbors should track committed position after motion", failures)

func _test_holder_fixture_waiting_grid_neighbors_exclude_waiting_position_after_motion(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	simulation.tick_bucket_with_report(store, 0.1, 0, 1)
	var waiting_pos := Vector2(store.position_x[1], store.position_y[1])
	_assert_true(not grid.query_neighbors(waiting_pos).has(1), "waiting grid neighbors should exclude waiting position after motion", failures)


func _test_holder_fixture_success_grid_presence_matches_nonzero_velocity(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	simulation.tick_bucket_with_report(store, 0.1, 0, 1)
	var committed_pos := Vector2(store.position_x[0], store.position_y[0])
	_assert_true(grid.query_neighbors(committed_pos).has(0), "success grid presence should stay visible after motion", failures)
	_assert_true(absf(float(store.velocity_x[0])) > 0.0 or absf(float(store.velocity_y[0])) > 0.0, "success velocity should stay non-zero after motion", failures)

func _test_holder_fixture_waiting_grid_absence_matches_zero_velocity(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
...
func _test_holder_fixture_assignment_anchor_matches_target_store_position_after_grid_update(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	store.intent_state[0] = Types.INTENT_STATE_ATTACK
	var report: Dictionary = simulation.tick_bucket_with_report(store, 0.1, 0, 1)
	var target_pos := Vector2(store.position_x[2], store.position_y[2])
	_assert_eq(report.get("assignments", {}).get(0, {}).get("global_pos", null), target_pos, "assignment anchor should match target store position after grid update", failures)
	_assert_eq(report.get("assignments", {}).get(1, {}).get("global_pos", null), target_pos, "waiting assignment anchor should match target store position after grid update", failures)

func _test_holder_fixture_assignment_anchor_remains_shared_across_attackers_after_grid_update(failures: Array[String]) -> void:
	var store = EntityStore.new(3)
	var grid = SpatialGrid.new(10.0)
	var simulation = BattleSimulationV4.new(grid)
	_prepare(store, 0, 0, Vector2(-1.0, 0.0), 6.0)
	...
	store.intent_state[0] = Types.INTENT_STATE_ATTACK
	var assignments: Dictionary = simulation.tick_bucket_with_report(store, 0.1, 0, 1).get("assignments", {})
	_assert_eq(assignments.get(0, {}).get("global_pos", null), assignments.get(1, {}).get("global_pos", null), "assignment anchor should stay shared across attackers after grid update", failures)
