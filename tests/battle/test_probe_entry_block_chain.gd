...
func run() -> Array[String]:
	...
	if block_to_entry_index != -1 and entry_to_fingerprint_index != -1:
		_assert_true(entry_to_fingerprint_index > block_to_entry_index, "probe entry-to-fingerprint transition should stay after block-to-entry ordering", failures)
