...
func run() -> Array[String]:
	...
	_assert_true(float(orbit.get("late_commit_deviation", 0.0)) > float(corridor.get("late_commit_deviation", 0.0)) and float(corridor.get("late_commit_deviation", 0.0)) > float(funnel.get("late_commit_deviation", 0.0)), "aggregation contract should preserve late commit deviation gradient", failures)
