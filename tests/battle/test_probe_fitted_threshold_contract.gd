...
func run() -> Array[String]:
	...
	_assert_true(source.contains('"fitted_from_sample_count": 20'), "sampling should persist fitted threshold sample count derived from real critical family runs", failures)
	_assert_true(source.contains('"warning_threshold_source": "old_escape_hit==true/p95_contention"'), "sampling should persist warning threshold source", failures)
	_assert_true(source.contains('"error_threshold_source": "old_escape_hit==true/p95_contention"'), "sampling should persist error threshold source", failures)
	_assert_true(source.contains('"old_escape_true_count"'), "sampling should persist old escape true count for threshold fitting", failures)
	_assert_true(source.contains('"old_escape_true_p95_contention_values"'), "sampling should persist old escape p95 contention samples", failures)
