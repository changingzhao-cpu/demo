...
func run() -> Array[String]:
	...
	if runner_banner_index != -1 and contract_ordering_chain_index != -1:
		_assert_true(contract_ordering_chain_index > runner_banner_index, "probe contract ordering chain should stay after the main runner banner contract", failures)
	return failures
