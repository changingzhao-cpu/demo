...
func _test_scene_conveys_side_angle_arena_readability(failures: Array[String]) -> void:
	...
	_assert_true(arena_shadow is CanvasItem and arena_shadow.visible, "battle scene should expose a visible ArenaShadow node for side-angle depth", failures)
	_assert_true(arena_floor is CanvasItem and arena_floor.visible, "battle scene should expose a visible ArenaFloor node for elliptical arena readability", failures)
